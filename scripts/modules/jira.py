...
def query_jira(jira_id):
  ...
  if "fields" in response:
    summary = response["fields"]["summary"].decode('unicode-escape').encode("utf-8")
    return "[{0}] {1} - {2}".format(jira_id, summary, color(jboss_org_case.replace(jboss_org_base_url,jboss_org_base_url_dns) + jira_id , colors.GREY))
  else:
